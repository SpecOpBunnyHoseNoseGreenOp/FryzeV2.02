...
do local row=gmsAimKeyCtrl.Row for _,c in ipairs(row:GetChildren()) do if c:IsA("TextLabel") then c.Size=u2(1,-41,1,0) elseif c:IsA("TextButton") then c.Size=u2(0,36,0,22) c.Position=u2(1,-38,0.5,-11) end end end
...
local hmTargetNames = {["HitMarker"]=true,["1"]=true,["2"]=true,["3"]=true,["4"]=true,["5"]=true,["6"]=true}mT(anS,22,"Anti Flashbang",false,function(v) sT.AntiFlashbang=v end) mT(anS,46,"Anti Smoke",false,function(v) sT.AntiSmoke=v; if v then for _,d in ipairs(workspace:GetDescendants()) do if d:IsA("ParticleEmitter") and (d.Name:lower():find("smoke") or d.Name:lower():find("gas")) then d.Enabled=false end end if not sC then sC=workspace.DescendantAdded:Connect(function(d) if sT.AntiSmoke and d:IsA("ParticleEmitter") and (d.Name:lower():find("smoke") or d.Name:lower():find("gas")) then d.Enabled=false end end) table.insert(conns,sC) end end end) mT(anS,70,"Anti Wallbang",false,function(v) sT.AntiWallbang=v end)
...
local cCF = workspace:WaitForChild("Characters",5)
...
local function gCM()
    ...
    if cCF then for _,m in ipairs(cCF:GetChildren()) do if m.Name~=lp.Name then eval(m) end end return c end
...
local function rE(p) local o=espI[p] if o then for k,v in pairs(o) do if k=="CornerBoxes" or k=="Bones" then for _,e in pairs(v) do pcall(function() if e and type(e.Remove)=="function" then e:Remove() end end) end else if v then pcall(function() if type(v.Remove)=="function" then v:Remove() end end) end end espI[p]=nil end end
    ...
    local function saGetTarget(origin)
        ...
        for _, player in ipairs(P:GetPlayers()) do
            if player ~= lp then
                ...
                if saIsAlive(character) then
                    if not (sT.SATeamCheck and player:GetAttribute("Team") == localTeam) then
                        ...
                        if part then
                            ...
                            if angle < bestAngle then
                                if not (sT.SAWallCheck and saIsObstructed(origin, part.Position, character)) then
